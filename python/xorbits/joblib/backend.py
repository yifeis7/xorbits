...
class XorbitsDistributedBackend(AutoBatchingMixin, ParallelBackendBase):
    ...
    def __init__(self, service=None, session=None, backend=None, n_parallel=None):
        ...
        if session is None:
            if service is not None:
                self.session = deploy.session.new_session(
                    service, backend=backend, default=False
                )
            else:
                self.session = deploy.session.get_default_session()
        else:
    ...
    def get_nested_backend(self):
        return XorbitsDistributedBackend(session=self.session), -1
    ...
    def effective_n_jobs(self, n_jobs):
        eff_n_jobs = super(XorbitsDistributedBackend, self).effective_n_jobs(n_jobs)
        if n_jobs == -1:
    ...
    def apply_async(self, func, callback=None):
        ...
        def f():
            ...
            # ret = (
            #     ExecutableTuple(spawned)
            #     .execute(session=self.session)
            #     .fetch(self.session)
            # )
            run(spawned)
            ret = [r.to_object() for r in spawned]
            return ret
...
def register_xorbits_backend():
    register_parallel_backend("xorbits", XorbitsDistributedBackend)
